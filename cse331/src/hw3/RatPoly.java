...
public final class RatPoly {
    ...
    public RatPoly(RatTerm rt) {
		 terms = new ArrayList<RatTerm>();
		 if (!rt.equals(RatTerm.ZERO) && rt.getExpt() >= 0) {
			 terms.add(rt);
		 }
		 checkRep();
    ...
    public RatTerm getTerm(int deg) {
		 ...
		 return RatTerm.ZERO;
        // TODO: Fill in this method, then remove the RuntimeException
        //throw new RuntimeException("RatPoly->getTerm() unimplemented!");
    }
    ...
    public boolean isNaN() {
		 if (terms.size() != 0) {
			 return terms.get(0).isNaN();
		 }
		 return false;
        // TODO: Fill in this method, then remove the RuntimeException
        //throw new RuntimeException("RatPoly->isNaN() unimplemented!");
    }
    ...
    private static void scaleCoeff(List<RatTerm> lst, RatNum scalar) {
		 for (int i = 0; i < lst.size(); i++) {
			 RatTerm current = lst.remove(i);
			 RatNum newCoeff = scalar.mul(current.getCoeff());
			 lst.add(i, new RatTerm(newCoeff, current.getExpt()));
		 }
        // TODO: Fill in this method, then remove the RuntimeException
    ...
    private static void incremExpt(List<RatTerm> lst, int degree) {
		 for (int i = 0; i < lst.size(); i++) {
			 RatTerm currentTerm = lst.remove(i);
			 RatTerm newTerm = new RatTerm(currentTerm.getCoeff(), currentTerm.getExpt() + degree);
			 lst.add(i, newTerm);
		 }
        // TODO: Fill in this method, then remove the RuntimeException
    ...
    private static void sortedInsert(List<RatTerm> lst, RatTerm newTerm) {
		 for (int i = 0; i < lst.size(); i++) {
			 RatTerm currentTerm = lst.get(i);
			if (currentTerm.getExpt() == newTerm.getExpt()) {
				newTerm = currentTerm.add(newTerm);
				lst.remove(i);
				if (!newTerm.isZero()) {
					lst.add(i, newTerm);
				}
				return;
			} else if (currentTerm.getExpt() < newTerm.getExpt()) {
				lst.add(i, newTerm);
				return;
			}
		 }
        ...
        // TODO: Fill in this method, then remove the RuntimeException
       // throw new RuntimeException("RatPoly->sortedInsert() unimplemented!");
    }
    ...
    public RatPoly negate() {
		 if (this.isNaN()) {
			 ...
			 for (int i = 0; i < terms.size(); i++) {
				 newTerms.add(terms.get(i).negate());
			 }
    ...
    public RatPoly add(RatPoly p) {
		 if (this.isNaN() || p.isNaN()) {
			 ...
			 //add each term from this to the new list.
			 for (int i = 0; i < terms.size(); i++) {
				 newTerms.add(terms.get(i));
			 }
			 //add each term from p using sorted insert.
			 List<RatTerm> pTerms = p.terms;
			 for (int i = 0; i < pTerms.size(); i++) {
				 RatPoly.sortedInsert(newTerms, pTerms.get(i));
			 }
			 return new RatPoly(newTerms);
		 }
        // TODO: Fill in this method, then remove the RuntimeException
        //throw new RuntimeException("RatPoly->add() unimplemented!");
    }
    ...
    public RatPoly mul(RatPoly p) {
		 if (this.isNaN() || p.isNaN()) {
			 ...
			 //for each term in this, multiply with each term of p and add to new list.
			 for (int i = 0; i < terms.size(); i++) {
				 RatTerm term = terms.get(i);
				 for (int j = 0; j < pTerms.size(); j++) {
					 RatTerm otherTerm = pTerms.get(j);
					 RatPoly.sortedInsert(newTerms, term.mul(otherTerm));
				 }
			 }
			 return new RatPoly(newTerms);
		 }
    ...
    public RatPoly div(RatPoly p) {
		 if (isNaN() || p.isNaN() || p.equals(RatPoly.ZERO)) {
			 return RatPoly.NaN;
		 } else {
			 List<RatTerm> listP = p.terms;
			 RatPoly numerator = new RatPoly(terms);
			 List<RatTerm> newTerms = new ArrayList<RatTerm>();

			 //first coefficient of p
			 RatNum pCoeff = listP.get(0).getCoeff();
			 while (!numerator.equals(RatPoly.ZERO) && numerator.degree() >= p.degree()) {
				 //create the term that will be added to the new polynomial.
				 RatNum numCoeff = numerator.terms.get(0).getCoeff();
				 RatNum newCoeff = numCoeff.div(pCoeff); 
				 int newExpt = numerator.degree() - p.degree();
				 RatTerm newTerm = new RatTerm(newCoeff, newExpt);
				 newTerms.add(newTerm);

				 //subtract a modified p from numerator, so numerator loses degrees.
				 RatPoly modP = p.mul(new RatPoly(newTerm));
				 numerator = numerator.sub(modP);
			 }
			 return new RatPoly(newTerms);
		 }
        // TODO: Fill in this method, then remove the RuntimeException
        //throw new RuntimeException("RatPoly->div() unimplemented!");
    }
    ...
    public RatPoly differentiate() {
		 ...
		 for (int i = 0; i < terms.size(); i++) {
			 RatTerm derivative = terms.get(i).differentiate();
			 if (!derivative.isZero()) {
				 newTerms.add(derivative);
			 }
		 }
		 return new RatPoly(newTerms);
        // TODO: Fill in this method, then remove the RuntimeException
    ...
    public RatPoly antiDifferentiate(RatNum integrationConstant) {
		 ...
		 if (this.isNaN() || integrationConstant.isNaN()) {
			 ...
			 for (int i = 0; i < terms.size(); i++) {
				 newTerms.add(terms.get(i).antiDifferentiate());
			 }
			 if (!integrationConstant.equals(RatNum.ZERO)) {
				 newTerms.add(new RatTerm(integrationConstant, 0));
			 }
		 }
		 return new RatPoly(newTerms);
        // TODO: Fill in this method, then remove the RuntimeException
    ...
    public double integrate(double lowerBound, double upperBound) {
		 ...
		 RatPoly antiderivative = this.antiDifferentiate(new RatNum(0));
