...
* Video関連メソッド
  ...
  * ((<SDL::Surface#destroy>)) -- サーフェスを解放します。
  * ((<SDL::Surface#destroyed?>)) -- サーフェスが解放されているかどうかを返します。
  ...
  * ((<SDL::Surface#pitch>)) -- サーフェスの各スキャンラインのバイト数
...
== 高速な描画のために
Ruby/SDLでゲームを作る場合を考えると、性能上のボトルネックとなるのは
描画部分が主になります。そこで、この部分を高速化することで
全体を高速化することができます。以下に描画を高速化するための
こつをいくつか書いておきます。

* ((<SDL::Surface#display_format>))を使う
  
  あらかじめこのメソッドを呼んでおいて高速描画可能なフォーマット
  に変更しておくと、劇的に高速化します。
...
* 描画には 
  ((<SDL::Surface.blit>)), ((<SDL::Surface#put>)), ((<SDL::Surface#fill_rect>)) のみを
  使う。
  
  まともに高速化されているのは以上の描画メソッドのみです。
  
* 回転や拡大縮小は描画時にせず、初期化時にそのような((<SDL::Surface>))を
  あらかじめ準備しておく

  回転や拡大縮小描画は上の項に書いた通り遅いので、性能が必要な
  場合にはあらかじめ回転拡大縮小して、((<SDL::Surface#display_format>))
  で高速な形式に変換しておいて、それを使うべきです。

また、アルファブレンドや回転や拡大縮小を高速に行いたいので
あれば、Video subsystem を使わずに OpenGL を使うという方法
もあります。高速なビデオカードとドライバを
用意できるのであれば Video subsystem を使うより
高速な描画が可能となるでしょう。
...
--- SDL::Surface#destroy
    ...
    そのSurfaceが占めているリソースを解放します。解放した後には
    そのオブジェクトに対する操作は一切不可能になります。
...
--- SDL::Surface#destroyed?
    ...
    そのサーフェスが ((<SDL::Surface#destroy>)) によって解放されているか
    どうかを真偽値で返します。
...
--- SDL::Surface#pitch
    ...
    サーフェスの各スキャンラインに使われるバイト数を返します。
    ((<SDL::Surface#pixels>)) で得られる画像データの解釈に必要となります。
    おおよそ ((<SDL::Surface#bpp>)) * ((<SDL::Surface#w>)) / 8 の値となりますが、
    パディングのため少しこれより大きくなる場合があります。
    
    ((<SDL::Surface#pixels>)) で得られるデータのバイト数は ((<SDL::Surface#h>)) * pitch と
    なります。
...
--- SDL::Surface.transform_draw(src,dst,angle,xscale,yscale,px,py,qx,qy,flags)
    ...
    * NOTES
      ...
      SDL::Surface::TRANSFORM_AA を使う場合は、高さ方向および幅方向にそれぞれ
      1ピクセルづつ間引かれます(パフォーマンスを良くするためです)。
      
      ((|src|))が32bpp RGBA フォーマットの場合には、SDL::Surface::TRANSFORM_SAFEを使うべきです。
...
--- SDL::Surface.transformBlit(src,dst,angle,xscale,yscale,px,py,qx,qy,flags)
    ...
    ((<SDL::Surface.transform_draw>)) と同様です。
    ((<SDL::Surface.transform_draw>)) との違いは ((|src|)) に設定されたカラーキー(抜き色)
...
--- SDL::Surface#transformSurface(bgcolor,angle,xscale,yscale,flags)
    ...
    ((<SDL::Surface.transform_draw>)) と同じ意味である。
...
* Event関連メソッド
  ...
  * ((<SDL::Event::JoyButtonUp#which>)) -- ジョイスティックデバイスの番号を返します。
...
--- SDL::Event.wait
    ...
    * NOTES
      ...
      Ruby 1.9ではイベントを待っている間に他のRubyのスレッド
      に切り替わることが可能です。
...
* ((<CD-ROM関連メソッド>))
  ...
  * ((<SDL::CD#close>)) -- CDデバイスをクローズします。
  * ((<SDL::CD#closed?>)) -- CDデバイスがクローズされたかどうかを返します。
...
--- SDL::CD#closed?
    ...
    ((<SDL::CD#close>))によってデバイスがクローズされたかどうかを
    返します。
...
* Audio関連メソッド
  ...
  * ((<SDL::Mixer::Wave#destroy>)) -- オーディオデータを解放します
  * ((<SDL::Mixer::Wave#destroyed?>)) -- オーディオデータを解放したかどうかを返します。
  * ((<SDL::Mixer::Music.load>)) -- ファイルから音楽データをロードする。
  * ((<SDL::Mixer::Music.load_from_string>)) -- 文字列から音楽データをロードする。
  * ((<SDL::Mixer::Music#destroy>)) -- 音楽データを解放します
  * ((<SDL::Mixer::Music#destroyed?>)) -- 音楽データを解放したかどうかを返します。
...
* フォント関連メソッド
  ...
  * ((<SDL::TTF#close>)) -- フォントをクローズします。
  * ((<SDL::TTF#closed?>)) -- フォントがクローズされたかどうかを返します。
  ...
  * ((<SDL::TTF#draw_blended_utf8>)) -- 文字列をアルファブレンディングでアンチエイリアスして描画します。
  * ((<SDL::TTF#render_solid>)) -- 文字列をレンダリングしたサーフェスを返します。
  * ((<SDL::TTF#render_shaded>)) -- 文字列をアンチエイリアシングしてレンダリングしたサーフェスを返します。
  * ((<SDL::TTF#render_blended>)) -- 文字列をアルファを使ってアンチエイリアスしたサーフェスを返します。
  * ((<SDL::TTF#draw_solid>)) -- 文字列を描画します。
  * ((<SDL::TTF#draw_shaded>)) -- 文字列をアンチエイリアス描画します。
  * ((<SDL::TTF#draw_blended>)) -- 文字列をアルファブレンディングでアンチエイリアスして描画します。
  * ((<SDL::BMFont.open>)) -- フォントをロードします。
  * ((<SDL::BMFont#close>)) -- フォントを閉じます。
  * ((<SDL::BMFont#closed?>)) -- フォントが閉じられているかどうかを返します。
  ...
  * ((<SDL::Kanji#close>)) -- 読みこんだフォントを閉じます。
  * ((<SDL::Kanji#closed?>)) -- フォントを閉じたかどうかを返します。
  * ((<SDL::Kanji#add>)) -- 既に読み込んでいるフォントに追加してフォントを読みこみます。
  * ((<SDL::Kanji#set_coding_system>)) -- 文字コードを指定します。
  * ((<SDL::Kanji#get_coding_system>)) -- 既定の文字コード返します。
...
--- SDL::TTF#closed?
    ...
    フォントが((<SDL::TTF#close>))でクローズされたかどうかを
    返します。
...
--- SDL::TTF#renderSolid(text, r, g, b)
    ...
    ((<SDL::TTF#render_solid_utf8>))と同じです。
...
--- SDL::TTF#renderShaded(text, r, g, b)
    ...
    ((|text|))がUTF8でなくても良いこと以外、
    ((<SDL::TTF#render_shaded_utf8>))と同じです。
    ((|text|))のエンコードは適切に変換されます。
...
--- SDL::TTF#renderBlended(text, r, g, b)
    ...
    ((|text|))がUTF8でなくても良いこと以外、
    ((<SDL::TTF#render_blended_utf8>))と同じです。
    ((|text|))のエンコードは適切に変換されます。
...
--- SDL::TTF#drawShaded(dest, text, r, g, b)
    ...
    ((|text|))がUTF8でなくても良いこと以外、
    ((<SDL::TTF#draw_shaded_utf8>))と同じです。
    ((|text|))のエンコードは適切に変換されます。
...
--- SDL::TTF#drawBlended(dest, text, r, g, b)
    ...
    ((|text|))がUTF8でなくても良いこと以外、
    ((<SDL::TTF#draw_blended_utf8>))と同じです。
    ((|text|))のエンコードは適切に変換されます。
...
--- SDL::BMFont#closed?
    ...
    ビットマップフォントが((<SDL::BMFont#close>))によって閉じられたか
    どうかを返します。
...
--- SDL::Kanji#closed?
    ...
    ((<SDL::Kanji#close>))でフォントを閉じたかどうかを返します。
...
--- SDL::CollisionMap.boundingBoxCheck(x1,y1,w1,h1,x2,y2,w2,h2)
    ...
    左上が(((|x1|)),((|y1|)))、幅((|w1|))高さ((|h1|))の長方形と
    左上が(((|x2|)),((|y2|)))、幅((|w2|))高さ((|h2|))の長方形が
    重なっているかどうかを判定します。
