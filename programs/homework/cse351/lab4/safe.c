...
void* mm_malloc (size_t size) {
  ...
  // explicit list.
  newPtr = POINTER_ADD(ptrFreeBlock, reqSize);
  newPtr->sizeAndTags = (SIZE(ptrFreeBlock->sizeAndTags) - reqSize) | TAG_PRECEDING_USED;
  ptrFreeBlock->sizeAndTags = reqSize | TAG_USED;
  insertFreeBlock(newPtr);

  // Sets footer
  *((int*)POINTER_ADD(newPtr, SIZE(newPtr->sizeAndTags) - WORD_SIZE)) = newPtr->sizeAndTags;
  coalesceFreeBlock(newPtr);
...
void mm_free (void *ptr) {
  ...
  followingBlock = POINTER_ADD(blockInfo, SIZE(blockInfo->sizeAndTags));
  followingBlock->sizeAndTags = followingBlock->sizeAndTags ^ TAG_PRECEDING_USED;
